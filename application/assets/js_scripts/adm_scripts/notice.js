...
/*delete notice*/
   $(document).on("click", ".deletenotice", function() {
     ...
     /*update published message notice*/
   $(document).on("click", ".updtaboutusbtn", function() {
      var pubmsgid = $(this).data("pubmsgid");
      var columnname = $(this).data("columnname");
      var message = $("#message").val();
    
      var urlpath = basepath + 'notice/updatePublishedMessage';
     
        $.ajax({
      type: "POST",
      url:  urlpath,
      data: {pubmsgid:pubmsgid,columnname:columnname,message:message},
      dataType: 'json',
      contentType: "application/x-www-form-urlencoded; charset=UTF-8", 
      success: function (result) {
        if(result.msg_status=1)
   ...
   // Set Status
    $(document).on("click", ".pubmsgtatus", function() {
    var uid = $(this).data("pubmsgid");
        var status = $(this).data("setstatus");
        var url = basepath + 'notice/setStatusPublishMsg';
        setActiveStatus(uid, status, url);
