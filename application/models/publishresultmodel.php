...
public function inserIntoResultListUpload($publishID,$data,$sesion_data)
	{
		try
		{  pre($data);
        	$this->db->trans_begin();
...
/**/
public function updateResultListUpload($publishID,$data,$sesion_data)
	{
		try
		{
        	$this->db->trans_begin();
			...
			$upd_where = array("result_publish.id" => $data['publishID']);
			...
			$this->db->where($upd_where);
            $this->db->update('result_publish',$insert_student_data);

			$insert_where = array(
				"masterID" => $data['publishID'],
				"From" => "ResultList",
			);
			...
			if($is_file_uploaded=="Y")
			{
				...
				if($data['mode']=="Edit" && $data['publishID']>0)
					{
			...
			$insert_user_activity = array(
				"activity_date" => date('Y-m-d'),  
				"activity_module" => 'Publish Result',
				
				"action" => "Update",
				"from_method" => "publishresult/saveResultList",
				"user_id" => $sesion_data['userid'],
...
public function insertIntoUploadFile($data,$session_data,$where_data)
	{ 
		...
		//$dir1 = $_SERVER['DOCUMENT_ROOT'].'/application/assets/ds-documents/resultlist_upload'; //server

		$dir1 = $_SERVER['DOCUMENT_ROOT'].'/sishubharati/application/assets/ds-documents/resultlist_upload'; //local
	...
	public function getResultListLatest($sessionid)
	{
		$data = array();
		$where = array(
						'result_publish.session_id' =>$sessionid, 
						'result_publish.is_visable_web' =>'Y'
					);
		$this->db->select("*")
				->from('result_publish')
				->where($where)
				->order_by('result_publish.id','desc')
				->limit(1);
		$query = $this->db->get();
		//echo $this->db->last_query();
