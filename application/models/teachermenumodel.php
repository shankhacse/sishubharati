...
class teachermenumodel extends CI_Model{
	public function getAllAdministrativeMenu($table)
	{
		$data = array();
		$where_Ary = array(
			"teacher_menu_master.is_parent" => "P",
			"teacher_menu_master.is_active" => "Y"
		);
		
		$this->db->select("*")
				->from($table)
				->where($where_Ary)
				->order_by('teacher_menu_master.menu_srl','ASC');
		$query = $this->db->get();
		if ($query->num_rows() > 0) 
		   {
	...
	public function getSecondLevelMenu($parentID,$table)
	{
		$data = array();
		$where_Ary = array(
			"teacher_menu_master.parent_id" => $parentID,
			"teacher_menu_master.is_active" => "Y"
		);
	...
	{
		$data = array();
		$sql = "SELECT * FROM teacher_menu_master WHERE teacher_menu_master.`menu_title`='".$menuTitle."' AND teacher_menu_master.`is_parent`='P'";
