...
class noticemodel extends CI_Model{
	...
	public function getAllNoticeList(){
		$data = [];
		$where = array('uploaded_documents_all.upload_from_module' =>'Notice' );
...
/* get all Notice List*/
	...
	public function inserIntoNotice($data,$sesion_data)
	{
		try
		{
			...
			$notice_insert = $this->db->insert('notice', $insert_notice_data);
...
public function insertIntoUploadFile($data,$session_data,$where_data)
	{ 
		if($data['mode']=="EDIT" && $data['noticeID']>0)
		{

			$where_notice = array(
				"uploaded_documents_all.upload_from_module_id" => $data['noticeID'],
		...
		$dir1 = $_SERVER['DOCUMENT_ROOT'].'/application/assets/ds-documents/notice_upload'; //server

		//$dir1 = $_SERVER['DOCUMENT_ROOT'].'/sishubharati/application/assets/ds-documents/notice_upload'; //local
